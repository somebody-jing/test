...
def ToSearchsensitive(sentence_list, rules_list, rules_split_list, pinyinlist, sensitive_words):
    ...
    for line, sentence in enumerate(sentence_list):
        sensitive_index = []
        for num, rules in enumerate(rules_split_list):
            for i in re.finditer(rules, sentence, re.I):  # 不区分大小写
                res = 'Line' + str(line) + ':' + ' <' + sensitive_words[num] + '> ' + i.group()+'\n'
                if num == 0:
                    a += 1
                elif num ==1:
                    b += 1
            ...
            for i in re.finditer(rules, sentence, re.I):
                ...
                for duoyinzi in location:
                    if i.span()[0] <= duoyinzi[0] <= i.span()[1]:
                        ...
                        flag = 0
                if flag == 1:
                    res = 'Line' + str(line + 1) + ':' + ' <' + sensitive_words[num] + '> ' + i.group()+'\n'  # group用来提出分组截获的字符串
                    if num == 0:
                        a += 1
                    elif num == 1:
                        b += 1
                    ...
                    ans = 'Line' + str(line + 1) + ':' + ' <' + sensitive_words[num] + '> ' + sentence[i.span()[0]:i.span()[1]] + '\n'
                    if num == 0:
                        a += 1
                    elif num == 1:
                        b += 1
                    ...
                    sensitive_index.append((i.span()[0], res))
                    count += 1
        sensitive_index.sort(key=takeFirst)
        # 将列表中的敏感词按下标位置输出
...
if __name__ == '__main__':
    ...
    txt_word = open(sys.argv[1], 'r', encoding='utf-8')
    txt_org = open(sys.argv[2], 'r', encoding='utf-8')
    txt_ans = open(sys.argv[3], 'w', encoding='utf-8')
    # 原文与敏感词读取
